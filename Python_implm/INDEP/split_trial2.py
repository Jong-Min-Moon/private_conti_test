...
for rep in range(nTests):
    ...
    z_og = generator_Y.sample((n,))
    data_y = cdf_calculator.cdf(y_og)   
    data_z = cdf_calculator.cdf(z_og)  
    ...
    nchunk = int(n/3)
    print(nchunk)
    data_dep = torch.cat((data_y[: nchunk, :], data_z[: nchunk, :]), 1)
    data_indep = torch.cat(( data_y[nchunk: 2 * nchunk, : ], data_z[2 * nchunk: , : ] ), 1)

    
    
    #

    test_results[rep], noise_vars[rep] = tester.run_test_conti_data(B, data_dep, data_indep,
                                             kappa, alpha, gamma,
                                             #################
                                             discrete = False
                                             ###############
