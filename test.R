...
multivariate.binning <- function(data.in, kappa){
  ...
  n = nrow(data)
  d = ncol(data)
  breaks = seq(from = 0, to = 1, length = kappa + 1)

  for (col in 1:d){
    data[, col] <- cut(data[, col], breaks = breaks, include.lowest = TRUE)
...
noise.conti <- function(n, d, kappa, alpha){
  ...
  noise <- rlaplace(n=n*kappa.d, m=0, s=1/sqrt(2))
  noise <- scale * noise
  return(noise)
}
...
permutation.test <- function(B, data.x, data.y, kappa, alpha, discrete = FALSE){
  n1 <- nrow(data.x)
  n2 <- nrow(data.y)
  d <- ncol(data.x)
  ...
  if(discrete){
    noise.x <-noise.discrete(n = n1, d = d,  kappa = kappa, alpha = alpha)
    
    noise.y <-noise.discrete(n = n2, d = d,  kappa = kappa, alpha = alpha)
    
  }else{
  noise.x <-noise.conti(n = n1, d = d,  kappa = kappa, alpha = alpha)
  noise.y <-noise.conti(n = n2, d = d,  kappa = kappa, alpha = alpha)
  }
  #print(mean(noise.x))
  #print(mean(noise.y))
  data.x <- kappa^{d/2} * data.x + noise.x
  data.y <- kappa^{d/2} * data.y + noise.y
  ...
  for (rep in 1:B){
    ...
    perm <- sample(1:(n1+n2)) # Question: same permutation can appear.
    data.permuted <- data.combined[perm, ]
    data.permuted.x <- data.permuted[1:n1, ]
    data.permuted.y <- data.permuted[(n1+1):(n1+n2), ]
    permutation.stats[rep] <- u.stat.two.sample(data.permuted.x, data.permuted.y)
  }
